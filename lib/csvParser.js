...
// Za Oracle: ukloni točke-tisućice, zamijeni decimalni zarez s točkom → "1234.56"
...
// Za JavaScript prikaz (parseFloat treba točku).
...
function buildClobParams(rows) {
  ...
  for (const cols of rows) {
    ...
    params.p_iznosd.push(normalizeDecimalOracle(cols[10]));
    params.p_iznosp.push(normalizeDecimalOracle(cols[11]));
    params.p_val.push(normalizeInt(cols[12]));
    params.p_iznval.push(normalizeDecimalOracle(cols[13]));
